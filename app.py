...
def sign_page(session_id, step):
    ...
    return render_template('sign.html', pdf=session_data['pdf'], session_id=session_id, step=step, fields=fields, email=fields[0]['email'])
...
def fill_field():
    ...
    if field['type'] == 'signature':
        ...
        apply_text(pdf_input_path, field['x'], field['y'], data['value'], scale)
...
def apply_signature(pdf_path, sig_data, output_path, x, y, scale=1.5):
    ...
    img_io.seek(0)
    can.drawImage(ImageReader(img_io), x_pdf, y_pdf, width=width/scale, height=height/scale, mask='auto')
    can.save()
    packet.seek(0)
...
Pour signer, cliquez ici : {app_url}/sign/{session_id}/{step}
""")
    try:
